import pygame
from constants import SCREEN_WIDTH as WIDTH, SCREEN_HEIGHT as HEIGHT, FPS, DIRS
from NN import NeuralNetwork
import random

class Individual:
    def __init__(self, x=None, y=HEIGHT-100, width=30, height=100, color=(100, 255, 100)):
        self.x = x or random.randint(50, WIDTH - 50)
        self.y = y
        self.width = width
        self.height = height
        self.color = color
        self.mutant = random.random() < 0.05
    ...
    def sense_environment(self, screen, platforms):
        ...
        for dx, dy in DIRS:
            ...
            while True:
                ...
                if hit:
                    # pygame.draw.line(screen, (255, 0, 0), (cx, cy), (x, y), 3)
                    break
            ...
            for j in range(self.brain.hidden_size):
                if (i + j) % 2:
                    if child1.mutant:
                        ...
                        child1.brain.w_hidden_output[i][j] = brain1.w_hidden_output[i][j]
                        
                    if child2.mutant:
                        child2.brain.w_hidden_output[i][j] = random.uniform(-1, 1)
                    else:
                        child2.brain.w_hidden_output[i][j] = brain2.w_hidden_output[i][j]
                else:
                    if child1.mutant:
                        child1.brain.w_hidden_output[i][j] = random.uniform(-1, 1)
                    else:
                        child1.brain.w_hidden_output[i][j] = brain2.w_hidden_output[i][j]
                        
                    if child2.mutant:
                        child2.brain.w_hidden_output[i][j] = random.uniform(-1, 1)
                    else:
                        child2.brain.w_hidden_output[i][j] = brain1.w_hidden_output[i][j]
